...
import Data.Vector.Mutable qualified as V (write)
...
pack ::
  -- | The requested size of the rectangle.
  Size ->
  -- | The 'Bin' from which to allocate.
  Bin ->
  -- | The bottom left coordinate of the allocated rectangle and the new 'Bin'.
  Maybe (Position, Bin)
 ...
 where
  ...
  guilMin1 pos free
    | rw <= sizeWidth free && rh <= sizeHeight free =
        PackM
          (sizeWidth free * sizeHeight free - rw * rh)
          pos
          bin
...
packArray ::
  -- | The requested size of the rectangle.
  Size ->
  -- | The 'BinArray' from which to allocate.
  BinArray ->
...
packArray size BinArray{..} = foldr (<|>) Nothing $ fmap tryPack [0 .. V.length binArrayBins - 1]
 where
  tryPack i = do
