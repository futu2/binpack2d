...
inFrameCheck (LargeSize bsize) (QC.InfiniteList szs _) = loop (emptyBin bsize) (take 1000 szs)
 where
  loop bin0 [] = True
  ...
  inFrame (Position x0 y0) (Size w0 h0) =
    (fromIntegral x0 :: Int) >= 0
      && (fromIntegral y0 :: Int) >= 0
      && x0 + w0 <= sizeWidth bsize
      && y0 + h0 <= sizeHeight bsize

noOverlapCheck (LargeSize bsize) (QC.InfiniteList szs _) = loop (emptyBin bsize) (take 1000 szs) []
 where
  loop bin0 [] rcs = True
...
acceptableRatioCheck (LargeSize bsize) (QC.InfiniteList szs _) = loop 0 (emptyBin bsize) (take 1000 szs)
 where
...
fullCheck (LargeSize bsize) (QC.InfiniteList szs _) =
  loop 0 (emptyBin bsize) (take 1000 szs ++ replicate (fromIntegral binArea) (Size 1 1))
 where
  area sz = sizeWidth sz * sizeHeight sz
  binArea = area bsize
  loop _ _ [] = False
